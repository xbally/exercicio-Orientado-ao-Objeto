...
public class MainLivroAutor {
    ...
    public void listarAutorComLivro() throws Exception {
        List<Autor> listaAutor = AutorDAO.listarAutorComLivro();
        Collections.sort(listaAutor, new Comparator<Autor>() {
            public int compare(Autor arg0) {
                String name = arg0.getNome();
                int i = name.compareToIgnoreCase(arg0.getNome());
                return i;
            }
        });
        System.out.println("ID\tTitulo do Livro\tAutores");
        for (Autor autor : listarAutores) {
            System.out.print(autor.getId() + "\t" + autor.getNome() + "\t");
        }
        for (Livro livro : livro.getTitulo()) {
